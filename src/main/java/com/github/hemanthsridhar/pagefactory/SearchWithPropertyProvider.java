 ...
 */
public class SearchWithPropertyProvider {

    private static Map<String, SearchWithPropertyProvider> providers = new ConcurrentHashMap<>();

    //this map is page -> (name -> locator))
    private Map<String, Map<String, By>> locators = new ConcurrentHashMap<>();

    private SearchWithPropertyProvider(String locatorsFile) throws IllegalArgumentException {
    ...
    public static SearchWithPropertyProvider getProvider(String locatorsFile) throws IllegalArgumentException {
        SearchWithPropertyProvider provider;

        if (!providers.containsKey(locatorsFile)) {
            provider = new SearchWithPropertyProvider(locatorsFile);
